...
if [[ -d "/usr/lib/jvm" ]]
then
    sudo rm -r /usr/lib/jvm && sudo mkdir /usr/lib/jvm
else 
...
do
    case $opt in
        "Java 8 (JDK)")
            ...
            export JAVA_BIN_DIR=/usr/lib/jvm/jdk1.8.0_261/bin
            cd ${JAVA_BIN_DIR}
...
then
    ...
    echo -e "\e[31mThe installation failed."
        ...
        "Java 8 (JRE)")
            ...
            export JAVA_BIN_DIR=/usr/lib/jvm/jre1.8.0_261/bin
            cd ${JAVA_BIN_DIR}
            a=(java javac javadoc javah javap javaws)
            for exe in ${a[@]}; do
            sudo update-alternatives --install "/usr/bin/${exe}" "${exe}" "${JAVA_BIN_DIR}/${exe}" 1
...
then
    ...
    echo -e "\e[31mThe installation failed."
        ...
        "Java 14")
            ...
            export JAVA_BIN_DIR=/usr/lib/jvm/jdk-14.0.2/bin
            cd ${JAVA_BIN_DIR}
            a=(java javac javadoc javah javap javaws)
            for exe in ${a[@]}; do
            sudo update-alternatives --install "/usr/bin/${exe}" "${exe}" "${JAVA_BIN_DIR}/${exe}" 1
...
then
    ...
    echo -e "\e[31mThe installation failed."
        ...
        "Java 11")
            ...
            export JAVA_BIN_DIR=/usr/lib/jvm/jdk-11.0.8/bin
            cd ${JAVA_BIN_DIR}
            a=(java javac javadoc javah javap javaws)
            for exe in ${a[@]}; do
            sudo update-alternatives --install "/usr/bin/${exe}" "${exe}" "${JAVA_BIN_DIR}/${exe}" 1
...
then
    ...
    echo -e "\e[31mThe installation failed."
            ...
            ;;
        "Java 8 32bit (JDK)")
            sudo tar zxvf jdk-8u261-linux-i586.tar.gz -C /usr/lib/jvm
...
then
    ...
    echo -e "\e[31mThe installation failed."
            ...
            ;;
        "Java 8 32bit (JRE)")
            sudo tar zxvf jre-8u261-linux-i586.tar.gz -C /usr/lib/jvm
